...
func buildRectPerimeterLoopAtGrid(g Grid, gx, gy, cols, rows int) Loop {
	n := 2*cols + 2*rows - 4
	tiles := make([]Tile, 0, n)
...
func loopRadius(n int, tileSize, pad float32) float32 {
	circ := float32(n) * (tileSize + pad)
